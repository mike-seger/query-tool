class QueryManager {
    ...
    configure() {
        ...
        if(configuration.customQueries) {
            document.getElementById('history-controls').style.display = "initial"
    ...
    async loadQueries() {
        try {
            ...
            listBox.innerHTML = Object.entries(queries)
                .map(([key, value]) => `<option value="${value.sql.trim()}">${key}</option>`)
                .join('')
    ...
    async runQuery(skipHistory = false) {
        ...
        try {
            ...
            this.pollCurrentQuery(
                !this.configuration.customQueries
                && query.minTTL && query.minTTL !== 0
            )
